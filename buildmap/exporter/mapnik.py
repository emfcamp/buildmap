...
class MapnikExporter(Exporter):
    ...
    def export(self):
        ...
        if self.config['symbol_path'] is not None:
            shutil.copytree(self.buildmap.resolve_path(self.config['symbol_path']),
        ...
        for layer_name, mml_file in mml_files:
            self.dest_layers[layer_name] = self.generate_mapnik_xml(layer_name, mml_file)
    ...
    def get_layer_css(self):
        ...
        for layer in self.config['raster_layer']:
            files.append(path.join(self.buildmap.resolve_path(self.config['stylesheet_path']),
                                   layer['stylesheet']))
    ...
    def write_mml_file(self, mss_file, source_layers):
        ...
        shutil.copyfile(mss_file, path.join(self.buildmap.temp_dir, path.basename(mss_file)))

...
class PowerPlugin(object):
    ...
    def generate_layers_config(self):
        ...
        if self.generator_layer and self.distro_layer and len(self.connection_layers) > 0:
            return True
        else:
            self.log.warn("Unable to locate all required power layers. Layers discovered: %s", layers)
            return False

    def get_generators(self):
        for row in self.db.execute(text("SELECT * FROM site_plan WHERE layer = :layer"),
                                   layer=self.generator_layer):
            yield Generator(row['ogc_fid'], get_key(row, 'generator'), get_key(row, 'name'))

    def get_distros(self):
        for row in self.db.execute(text("SELECT * FROM site_plan WHERE layer = :layer"),
                                   layer=self.distro_layer):
            yield Distro(row['ogc_fid'], get_key(row, 'distro'), get_key(row, 'name'), get_key(row, 'load'))
    ...
    def get_outbound_connections(self, ogc_fid):
        ...
        sql = text("""SELECT end_node.ogc_fid, edge.layer,
        ...
        for row in self.db.execute(sql, connection_layers=list(self.connection_layers.keys()),
                                   distro_layers=[self.distro_layer], start_fid=ogc_fid, buf=self.BUFFER):
    ...
    def generate_plan(self):
        ...
        while len(tree_nodes) > 0:
            ...
            for end_fid, layer, length in self.get_outbound_connections(start_fid):
                ...
                plan.add_connection(start_node, end_node, connection.I,
                                    connection.phases, length=length)
    ...
    def run(self):
        if not self.generate_layers_config():
            return
        self.log.info("Power layers detected. Generators: '%s', Distro: '%s', Connections: %s",
                      self.generator_layer, self.distro_layer, list(self.connection_layers.keys()))
